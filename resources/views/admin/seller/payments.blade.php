            ...
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    ...
                    <div class="bz-toolbar">
                        ...
                        <label for="credit_days" class="form-label mb-0">Credit Days</label>
                        <div class="bz-toolbar-field bz-toolbar-field--sm">
                            <input type="number" class="form-control @error('credit_days') is-invalid @enderror" id="credit_days" wire:model.defer="credit_days" placeholder="Credit Days" min="0">
                            @error('credit_days')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <button class="btn btn-danger btn-sm" wire:click="updateCreditAvailability()"><i class="bi bi-check-lg"></i>Update</button>
                ...
                <div class="card-body">
                    <div class="bz-stat-grid">
                        <div>
                            <a class="bz-stat {{ $mode == 'cashwallet' ? 'bz-stat--green' : 'bz-stat--slate' }}"
                                href="{{route('admin.seller.payments', $data->id)}}?mode=cashwallet" wire:navigate>
                                ...
                                <div class="bz-stat__value bz-num">₹ {{ formatIndianNumber($data->cash_balance) }}</div>
                            </a>
                            @if ($mode == 'cashwallet')
                                <button class="btn btn-secondary btn-sm mt-2" data-bs-toggle="modal"
                                    data-bs-target="#addBalance"><i class="bi bi-plus-lg"></i>Add Balance</button>
                            @endif
                        </div>
                        <div>
                            <a class="bz-stat {{ $mode == 'creditwallet' ? 'bz-stat--blue' : 'bz-stat--slate' }}"
                                href="{{route('admin.seller.payments', $data->id)}}?mode=creditwallet" wire:navigate>
                                <div class="bz-stat__top">
                                    <span class="bz-stat__label">Credit Balance</span>
                                    <span class="bz-stat__icon"><i class="bi bi-credit-card-2-front"></i></span>
                                </div>
                                <div class="bz-stat__value bz-num">₹ {{ formatIndianNumber($data->credit_balance) }}</div>
                            </a>
                    ...
                    <div class="table-responsive bz-section-gap">
                                ...
                                @if ($mode == 'cashwallet')
                                    @forelse ($cash_transactions as $cash_transaction)
                                        <tr>
                                            ...
                                            <td class="bz-num"><b>₹ {{formatIndianNumber($cash_transaction->amount)}}</b></td>
                                            <td>
                                ...
                                @if ($mode == 'creditwallet')
                                    @forelse ($credit_transactions as $credit_transaction)
                                        <tr>
                                            ...
                                            <td class="bz-num"><b>₹ {{formatIndianNumber($credit_transaction->amount)}}</b></td>
                                            <td>
